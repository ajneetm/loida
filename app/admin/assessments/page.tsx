  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="font-display text-3xl text-[#022269] font-light">Assessments</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        {assessments.map(a => {
          ...
          const questions = (JSON.parse(a.questions as string) as any[]) ?? []
          return (
            <div key={a.id} className="bg-white rounded-none border border-stone-100 p-6 hover:border-stone-200 transition-all">
              <div className="flex items-start justify-between mb-4">
                <div className="flex items-center gap-2">
                  ...
                  <span className={`text-[10px] px-2 py-0.5 rounded-none font-medium ${a.isActive ? 'bg-green-50 text-green-600' : 'bg-stone-100 text-stone-400'}`}>
                    {a.isActive ? 'Active' : 'Inactive'}
                  </span>
                </div>
                <Link href={`/admin/assessments/${a.id}/edit`}
                  className="text-xs text-stone-400 hover:text-[#c71430] transition-colors">
                  Edit →
                </Link>
              </div>

              <h3 className="font-display text-xl text-[#022269] font-medium mb-2">{a.title}</h3>
      ...
      {assessments.length === 0 && (
        <div className="text-center py-20 bg-white rounded-none border border-stone-100">
          ...
          <Link href="/admin/assessments/new"
            className="text-xs bg-[#022269] text-white px-5 py-2.5 rounded-none hover:bg-[#011344] transition-colors">
