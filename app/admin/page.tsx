...
export default async function AdminPage() {
  ...
  const stats = [
    { label: 'Total Users',       value: totalUsers,       color: '#c71430',  href: '/admin/users'    },
    ...
    { label: 'Enrollments',       value: totalEnrollments, color: '#c71430',  href: '/admin/programs' },
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="font-display text-3xl text-[#022269] font-light">Platform Overview</h1>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        {stats.map(s => (
          <Link key={s.label} href={s.href}
            className="bg-white rounded-none border border-stone-100 p-5 hover:border-stone-200 hover:shadow-sm transition-all relative group">
            {s.badge && (
              <span className="absolute -top-1.5 -right-1.5 w-5 h-5 bg-orange-500 text-white text-[9px] font-bold rounded-none flex items-center justify-center">
                {s.value}
              </span>
            )}
            <p className="font-display text-4xl text-[#022269] font-light group-hover:text-[#c71430] transition-colors">
              {s.value}
            </p>
            <p className="text-stone-400 text-xs mt-1">{s.label}</p>
            <div className="mt-3 h-0.5 w-8 rounded-none" style={{ background: s.color }} />
          </Link>
        ))}
      </div>
      ...
      {pendingCoaches > 0 && (
        <div className="bg-orange-50 border border-orange-100 rounded-none p-5 flex items-center justify-between">
          <div>
          ...
          <Link href="/admin/coaches?status=PENDING"
            className="bg-orange-500 hover:bg-orange-600 text-white text-xs font-medium px-4 py-2 rounded-none transition-colors flex-shrink-0">
            Review Now →
          </Link>
        </div>
      )}
      ...
      {/* Recent users */}
      <div className="bg-white rounded-none border border-stone-100">
        <div className="flex items-center justify-between p-6 border-b border-stone-50">
          <h2 className="font-display text-xl text-[#022269]">Recent Members</h2>
          <Link href="/admin/users" className="text-xs text-[#c71430] hover:underline">View all</Link>
        </div>
        <div className="divide-y divide-stone-50">
          {recentUsers.map(user => (
            <div key={user.id} className="flex items-center gap-4 px-6 py-4 hover:bg-stone-50/50 transition-colors">
              <div className="w-9 h-9 rounded-none bg-[#022269]/8 flex items-center justify-center text-[#022269] text-sm font-medium flex-shrink-0">
                {user.name?.charAt(0) ?? '?'}
              </div>
              ...
              <div className="hidden sm:flex items-center gap-2">
                <span className={`text-[10px] px-2 py-0.5 rounded-none font-medium border ${
