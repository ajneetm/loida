  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="font-display text-3xl text-[#022269] font-light">New Program</h1>
        <button onClick={() => router.back()} className="text-stone-400 hover:text-stone-600 text-sm">← Back</button>
      </div>

      <div className="bg-white rounded-none border border-stone-100 p-6 space-y-5">
          ...
          <div className="grid grid-cols-4 gap-2">
            {types.map(t => (
              <button key={t} type="button" onClick={() => setForm(p => ({ ...p, type: t }))}
                className={`py-2.5 rounded-none text-xs font-medium border transition-all ${form.type === t ? 'bg-[#022269] text-white border-[#022269]' : 'border-stone-200 text-stone-500 hover:border-stone-400'}`}>
      ...
      <div className="flex gap-3">
        <button onClick={() => handleSave(false)} disabled={saving}
          className="flex-1 border border-stone-200 text-stone-600 hover:border-stone-400 text-sm font-medium py-3 rounded-none transition-colors disabled:opacity-50">
          Save as Draft
        </button>
        <button onClick={() => handleSave(true)} disabled={saving}
          className="flex-1 bg-[#c71430] hover:bg-[#e8203c] text-white text-sm font-medium py-3 rounded-none transition-colors disabled:opacity-50">
