...
export default async function TrainerDashboard() {
  ...
  return (
    <div className="space-y-8">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-semibold text-[#1C2B39]">لوحة المدرب</h1>
        <p className="text-sm text-[#6B8F9E] mt-1">
          مسجّل عبر الوكيل: {trainer.agent.user.name}
        </p>
      </div>

      {/* Stats */}
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <StatCard icon={<BookOpen className="w-5 h-5" />} label="المناهج المعتمدة" value={activeAccreditations.length} />
        <StatCard icon={<FileText className="w-5 h-5" />} label="طلبات الشهادات" value={trainer.certificateRequests.length} />
        <StatCard icon={<Award className="w-5 h-5" />} label="قيد الانتظار" value={pendingCerts} color="amber" />
      </div>

      {/* Curricula */}
      <div className="space-y-3">
        <h2 className="font-medium text-[#1C2B39]">مناهجي</h2>
        {activeAccreditations.length === 0 ? (
          <div className="bg-white rounded-xl border border-[#E8E4DC] p-10 text-center text-[#6B8F9E]">
            ...
            <p>لم يتم اعتمادك على أي منهج بعد</p>
            <p className="text-xs mt-1">تواصل مع وكيلك لإضافة المناهج</p>
          </div>
        ) : (
      ...
      {trainer.certificateRequests.length > 0 && (
        <div className="bg-white rounded-xl border border-[#E8E4DC] overflow-hidden">
          <div className="px-6 py-4 border-b border-[#E8E4DC] flex items-center justify-between">
            <h2 className="font-medium text-[#1C2B39]">آخر طلبات الشهادات</h2>
    ...
    <div className="bg-white rounded-xl border border-[#E8E4DC] overflow-hidden">
      ...
      <div className="px-5 py-4 space-y-3">
        <div className="flex items-center justify-between text-sm">
          <span className="text-[#6B8F9E]">تاريخ الاعتماد</span>
          <span className="text-[#1C2B39]">
            {new Date(accreditation.createdAt).toLocaleDateString('ar')}
          </span>
        </div>
        <a
          href={accreditation.curriculum.siteUrl}
          target="_blank"
...
function StatusBadge({ status }: { status: string }) {
  const map: Record<string, { label: string; cls: string }> = {
    ...
    APPROVED: { label: 'موافق عليه',   cls: 'bg-blue-100 text-blue-700' },
    ISSUED:   { label: 'صدرت الشهادة', cls: 'bg-green-100 text-green-700' },
