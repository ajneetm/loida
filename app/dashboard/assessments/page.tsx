...
export default async function AssessmentsPage() {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-10">
      ...
      {results.length > 0 && (
        <div>
          <h2 className="font-display text-2xl text-[#022269] mb-5">Your Results</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {results.map(r => {
              const color = getDomainColor(r.assessment.domain)
              return (
                <div key={r.id} className="bg-white rounded-none border border-stone-100 p-6 flex gap-5">
                  {/* Score ring */}
                  <div className="flex-shrink-0 w-16 h-16 rounded-none border-2 flex items-center justify-center flex-col"
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <h3 className="text-[#022269] font-medium text-sm">{r.assessment.title}</h3>
                    <p className="text-stone-400 text-xs mt-1">{formatDate(r.completedAt)}</p>
                    {r.recommendations && (JSON.parse(r.recommendations as string) as string[]).length > 0 && (
                      <div className="mt-3 space-y-1">
                        {(JSON.parse(r.recommendations as string) as string[]).slice(0, 2).map((rec, i) => (
        ...
        return (
          <div key={domain}>
            <div className="flex items-center gap-3 mb-5">
              ...
              <h2 className="font-display text-2xl text-[#022269]">{getDomainLabel(domain)}</h2>
              ...
              {domainAssessments.map(assessment => {
                ...
                return (
                  <div key={assessment.id}
                    className="bg-white rounded-none border border-stone-100 p-6 hover:border-stone-200 hover:shadow-sm transition-all group">
                    <div className="flex items-start justify-between mb-4">
                      <div className="w-10 h-10 rounded-none flex items-center justify-center text-sm"
                        style={{ background: `${color}12`, color }}>
                        ◎
                      </div>
                      {done && (
                        <span className="text-[10px] bg-green-50 text-green-600 border border-green-100 px-2 py-1 rounded-none font-medium">
                          Completed ✓
                        </span>
                      )}
                    </div>

                    <h3 className="font-display text-xl text-[#022269] font-medium mb-2 group-hover:text-[#c71430] transition-colors">
                      {assessment.title}
                    </h3>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <Link
                        ...
                        className="text-xs font-medium px-4 py-2 rounded-none transition-all hover:opacity-90 text-white"
