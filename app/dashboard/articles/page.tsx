    ...
    <div className="max-w-5xl mx-auto space-y-8">
      ...
      <div className="bg-[#0A1628] rounded-2xl p-8 relative overflow-hidden">
        ...
        <div className="relative z-10">
          <p className="text-[#B8973A] text-xs tracking-[0.2em] uppercase mb-2">Insights & Articles</p>
          ...
          {articles.map(article => {
            ...
            return (
              <Link key={article.id} href={`/dashboard/articles/${article.slug}`}
                className="bg-white rounded-2xl border border-stone-100 overflow-hidden hover:border-stone-200 hover:shadow-md transition-all duration-300 group flex flex-col">
                {/* Color bar */}
                ...
                <div className="p-6 flex-1 flex flex-col">
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    {(JSON.parse(article.tags as string) as string[]).slice(0, 2).map(t => (
                      <span key={t} className="text-[10px] bg-stone-50 border border-stone-100 text-stone-400 px-2 py-0.5 rounded-full">
                        {t}
                      </span>
                    ))}
                  </div>
                  <h3 className="font-display text-xl text-[#0A1628] font-medium leading-snug mb-2 group-hover:text-[#B8973A] transition-colors">
                    {article.title}
                  </h3>
                  {article.excerpt && (
