...
export default async function ArticlePage({ params }: { params: { slug: string } }) {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8">
        ...
        <div className="p-8 md:p-12">
          ...
          <div className="flex items-center gap-2 mb-5">
            ...
            {(JSON.parse(article.tags as string) as string[]).map(t => (
              <span key={t} className="text-[10px] bg-stone-50 border border-stone-100 text-stone-400 px-2.5 py-1 rounded-none">
                {t}
              </span>
            ))}
          </div>
          ...
          {/* Title */}
          <h1 className="font-display text-4xl md:text-5xl text-[#022269] font-light leading-tight mb-4">
            {article.title}
          </h1>
          ...
          <div className="flex items-center gap-4 pb-8 border-b border-stone-100 mb-8">
            <div className="w-9 h-9 rounded-none bg-[#022269]/8 flex items-center justify-center text-[#022269] text-sm font-medium">
              {article.author.name?.charAt(0) ?? 'A'}
            </div>
            <div>
              <p className="text-[#022269] text-sm font-medium">{article.author.name}</p>
              {article.publishedAt && (
          ...
          {/* Content */}
          <div className="prose prose-stone max-w-none prose-headings:font-display prose-headings:font-light prose-a:text-[#c71430]">
      ...
      {related.length > 0 && (
        <div>
          <h2 className="font-display text-2xl text-[#022269] mb-5">Related Articles</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {related.map(r => (
              <Link key={r.id} href={`/dashboard/articles/${r.slug}`}
                className="bg-white rounded-none border border-stone-100 p-5 hover:border-stone-200 hover:shadow-sm transition-all group">
                <div className="h-0.5 w-8 rounded-none mb-4" style={{ background: getDomainColor(r.domain) }} />
                <h3 className="text-[#022269] font-medium text-sm leading-snug group-hover:text-[#c71430] transition-colors">
                  {r.title}
                </h3>
                {r.excerpt && (
