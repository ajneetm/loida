...
export default async function ProfilePage() {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      {/* Profile header */}
      <div className="bg-white rounded-none border border-stone-100 p-8 flex flex-col sm:flex-row items-start gap-6">
        {/* Avatar */}
        <div className="w-20 h-20 rounded-none bg-[#022269] flex items-center justify-center text-white text-2xl font-light font-display flex-shrink-0">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-4 flex-wrap">
            <div>
              <h2 className="font-display text-2xl text-[#022269] font-medium">{user.name ?? 'Member'}</h2>
              <p className="text-stone-400 text-sm mt-0.5">{user.email}</p>
            </div>
            <div className="flex items-center gap-2">
              <span className={`text-xs px-3 py-1.5 rounded-none font-medium border ${
            ...
            {user.profile?.interests && (JSON.parse(user.profile.interests as string) as string[]).length > 0 && (
              <div className="flex flex-wrap gap-1.5">
                {(JSON.parse(user.profile.interests as string) as string[]).map(i => (
                  <span key={i} className="text-[10px] bg-stone-100 text-stone-500 px-2 py-0.5 rounded-none">
                    {getDomainLabel(i)}
                  </span>
                ))}
              </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          ...
          <div key={s.label} className="bg-white rounded-none border border-stone-100 p-6 text-center">
            <p className="font-display text-4xl text-[#022269] font-light">{s.value}</p>
      ...
      {/* Edit profile form */}
      <div className="bg-white rounded-none border border-stone-100 p-8">
        <h3 className="font-display text-xl text-[#022269] mb-6">Edit Profile</h3>
        <ProfileEditForm user={user} />
      </div>

      {/* Account info */}
      <div className="bg-white rounded-none border border-stone-100 p-8">
        <h3 className="font-display text-xl text-[#022269] mb-5">Account</h3>
        <div className="space-y-4">
          {[
            ...
            <div key={row.label} className="flex items-center justify-between py-3 border-b border-stone-50">
              ...
              <p className="text-[#022269] text-sm font-medium">{row.value}</p>
            </div>
          ))}
        </div>
      </div>
