    ...
    <div className="max-w-5xl mx-auto space-y-8">
      ...
      <div className="bg-[#0A1628] rounded-2xl p-8 relative overflow-hidden">
        ...
        <div className="relative z-10">
          <p className="text-[#B8973A] text-xs tracking-[0.2em] uppercase mb-2">Expert Guidance</p>
      ...
      <div className="flex flex-wrap gap-2">
        <Link href="/dashboard/coaches"
          className={`px-4 py-2 rounded-full text-xs transition-all ${!domain ? 'bg-[#0A1628] text-white' : 'border border-stone-200 text-stone-500 hover:border-stone-400'}`}>
          All Coaches
        </Link>
        {domains.map(d => (
          <Link key={d} href={`/dashboard/coaches?domain=${d}`}
            className="px-4 py-2 rounded-full text-xs border transition-all hover:text-white"
            style={domain === d
      ...
      {coaches.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {coaches.map(coach => (
            <div key={coach.id} className="bg-white rounded-2xl border border-stone-100 p-6 hover:border-stone-200 hover:shadow-md transition-all duration-300 flex flex-col">
              {/* Avatar + info */}
              <div className="flex items-center gap-4 mb-4">
                <div className="w-14 h-14 rounded-2xl bg-[#0A1628] flex items-center justify-center text-white font-display text-xl flex-shrink-0">
                  {coach.user.name?.charAt(0) ?? 'C'}
                </div>
                <div className="min-w-0">
                  <h3 className="text-[#0A1628] font-medium text-sm">{coach.user.name}</h3>
              ...
              {(() => { const specs = JSON.parse(coach.specialties as string) as string[]; return specs.length > 0 ? (
                <div className="flex flex-wrap gap-1.5 mb-5">
                  {specs.slice(0, 3).map((s: string) => (
                    <span key={s} className="text-[10px] bg-stone-50 text-stone-500 border border-stone-100 px-2 py-0.5 rounded-full">
                      {s}
                    </span>
                  ))}
                </div>
              ...
              <div className="flex items-center justify-between mt-auto pt-4 border-t border-stone-50">
                ...
                <Link
                  ...
                  className="bg-[#0A1628] hover:bg-[#1A2B4A] text-white text-xs font-medium px-4 py-2 rounded-full transition-colors"
                >
                  Book Session
                </Link>
              </div>
      ...
      <div className="bg-stone-50 border border-stone-100 rounded-2xl p-6 text-center">
        <p className="text-[#0A1628] font-medium text-sm mb-1">Are you an expert in your field?</p>
        ...
        <Link
          ...
          className="inline-flex text-sm font-medium text-[#B8973A] border border-[#B8973A]/30 px-5 py-2 rounded-full hover:bg-[#B8973A] hover:text-white transition-all duration-300"
        >
          Apply as Coach →
        </Link>
      </div>
