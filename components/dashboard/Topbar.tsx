...
export default function DashboardTopbar({ user }: Props) {
  ...
  return (
    <header className="bg-white border-b border-stone-200 px-6 md:px-8 py-4 flex items-center justify-between sticky top-0 z-20">
      <div>
        <h1 className="font-display text-xl text-[#022269] font-medium">{title}</h1>
      ...
      <div className="flex items-center gap-3">
        <Link
          ...
          className="hidden sm:inline-flex bg-[#022269] hover:bg-[#011344] text-white text-xs font-medium px-4 py-2 rounded-none transition-colors"
        >
          + New Assessment
        </Link>
      </div>
